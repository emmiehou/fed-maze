/*
EDITED 4/7/25 to add SDcard logging for variables relevant to rule shifting  
...
void FED3::writeHeader() {
  ...
  if (tempSensor == false){
    logfile.println("MM:DD:YYYY hh:mm:ss,Library_Version,Session_type,Device_Number,Battery_Voltage,Motor_Turns,FR,Event,Active_Poke,Irrelevant_Cue,Rule,Outcome, Trial,Consec_Correct,Rule_Shift_Phase,Left_Poke_Count,Right_Poke_Count,Pellet_Count,Block_Pellet_Count,Retrieval_Time,InterPelletInterval,Poke_Time");
  }
...
void FED3::logdata() {
  ...
  /////////////////////////////////
  // Log which side the Irrelevant Cue is signaling
  /////////////////////////////////
  if (irrelevantCue == 0) logfile.print("Right"); //
  if (irrelevantCue == 1) logfile.print("Left"); //
  logfile.print(",");

  /////////////////////////////////
  // Log Sound Rule or Light Rule
  ////////////////////////////////
  if (sideRule == 1)  logfile.print("Side"); 
  if (soundRule == 1)  logfile.print("Sound"); 
  if (lightRule == 1)  logfile.print("Light");
  logfile.print(",");
  ...
  //////////////////////////////////
  // Log Rule Shift Phase
  //////////////////////////////////
  if (ruleshiftPhase == 0) { 
	logfile.print("IA");
  }
  else {
      logfile.print(ruleshiftPhase);
  }
  logfile.print(",");
...
void FED3::SelectMode() {
  ...
  //Otherwise we don't know them and just use Mode 1 through Mode 4
    ...
    if (FEDmode == 16) display.print("Mode 17");
    if (FEDmode == 17) display.print("Mode 18");
    if (FEDmode == 18) display.print("Mode 19");
    if (FEDmode == 19) display.print("Mode 20");
    if (FEDmode == 20) display.print("Mode 21");
    if (FEDmode == 21) display.print("Mode 22");
    display.refresh();
  }
